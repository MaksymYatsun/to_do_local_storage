document.addEventListener("DOMContentLoaded", () => {
  ...
  function initThemeChange() {
    const moonPath = "M16 28C16 43.1878 28 55.5 28 55.5C12.8122 55.5 0.5 43.1878 0.5 28C0.5 12.8122 12.8122 0.5 28 0.5C28 0.5 16 12.8122 16 28Z";
  ...
  function addToDo(toDoId) {
    ...
    function displayItems() {
      ...
      for (let i = 0; i < itemsArray.length; i++) {
        ...
        listItem.classList.add('card', 'border', 'relative', 'rounded-xl', 'p-2', 'bg-opacity-90', 'bg-slate-700', 'transition-all', 'hover:shadow-2xl', 'hover:shadow-red-500', 'focus-within:shadow-2xl', 'focus-within:shadow-red-600')
        items.appendChild(listItem);

        const text = document.createElement('textarea');
        text.classList.add('resize-none', 'bg-transparent', 'w-full', 'mb-3', 'h-[200px]', 'scrollbar-thin', 'scrollbar-thumb-slate-600', 'w-[80%]', 'outline-none');
        text.disabled = true;
        text.innerText = itemsArray[i];
        listItem.appendChild(text);

        const editItem = document.createElement('div');
        editItem.classList.add('absolute', 'px-2', 'right-0', 'top-2');
        ...
        deleteBtn.classList.add('fa-solid', 'fa-check', 'transition-all', 'hover:scale-110', 'focus:scale-110', 'outline-none');
        editItem.appendChild(deleteBtn);

        const editBtn = document.createElement('i');
        editBtn.dataset.btn = 'edit';
        editBtn.tabIndex = 0;
        editBtn.classList.add('fa-solid', 'fa-pen-to-square', 'transition-all', 'hover:scale-110', 'focus:scale-110', 'outline-none');
        ...
        saveBtn.classList.add('font-bold', 'mr-3', 'py-1', 'px-3', 'transition-all', 'hover:scale-110', 'focus:scale-110', 'outline-none')
        saveBtn.dataset.btn = 'save';
        saveBtn.innerText = 'Save';
        updateItem.appendChild(saveBtn);

        const cancelBtn = document.createElement('button');
        cancelBtn.classList.add('font-bold', 'py-1', 'px-3', 'transition-all', 'hover:scale-110', 'focus:scale-110', 'outline-none')
